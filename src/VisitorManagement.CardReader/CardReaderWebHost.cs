...
public static class CardReaderWebHost
{
    public static WebApplication Build(string[] args)
    {
        ...
        app.MapGet("/status", StatusPayload);
        app.MapGet("/api/status", StatusPayload);
    ...
    public static string GetListenUrl(IConfiguration config) =>
        config["CardReader:ListenUrl"] ?? "http://127.0.0.1:5001";

    private static IResult StatusPayload(IPcscReaderHub hub)
    {
        try
        {
            var probe = hub.Probe();
    ...
    private static IResult ReadCard(IPcscReaderHub hub, bool photo)
    {
        try
        {
